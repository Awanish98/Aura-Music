...
<aside
	...
	class={expanded
		? // ponytail: left offsets mirror Sidebar's w-16/lg:w-60 (and its manual collapse), right
			...
			`absolute inset-y-0 left-16 right-0 z-30 flex h-full flex-col border-l border-white/10 apple-glass shadow-2xl ${ui.sidebarCollapsed ? '' : 'lg:left-60'} ${queueOpen ? 'lg:right-80' : ''}`
		: `absolute inset-y-0 right-0 z-30 flex h-full w-80 max-w-[80vw] flex-col border-l border-white/10 apple-glass shadow-2xl ${queueOpen ? 'lg:right-80' : ''}`}
>
