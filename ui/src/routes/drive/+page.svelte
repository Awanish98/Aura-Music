<script lang="ts">
	...
	async function scanDrive() {
		...
		try {
			...
			toast.success(`Discovered ${scanned.length} songs from Google Drive!`);
		} catch (e: any) {
	...
	function playAll(shuffle = false) {
		...
		api.playPlaylist(items, 0, undefined, 'Google Drive Music', shuffle);
	}
...
<svelte:head>
	<title>Google Drive Music • Echo Music</title>
</svelte:head>

<div class="container mx-auto max-w-7xl px-4 py-6 md:px-8 space-y-6">
	<!-- Hero Banner -->
	<div class="relative overflow-hidden rounded-3xl border border-border/80 bg-gradient-to-br from-card via-card/70 to-primary/10 p-6 md:p-8 backdrop-blur-xl shadow-2xl">
		<div class="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
			<div class="flex items-center gap-5">
				<div class="flex h-16 w-16 md:h-20 md:w-20 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-500 text-white shadow-xl shadow-blue-500/25">
					<HugeiconsIcon icon={CloudIcon} class="h-8 w-8 md:h-10 md:w-10" />
				</div>
				<div>
					<div class="flex items-center gap-2">
						<h1 class="text-2xl md:text-3xl font-bold tracking-tight text-foreground">Google Drive Music</h1>
						{#if authState.connected}
							<span class="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-xs font-semibold text-emerald-500 border border-emerald-500/20">
								<HugeiconsIcon icon={CheckmarkCircle01Icon} class="h-3.5 w-3.5" /> Synced
							</span>
						{/if}
					</div>
					<p class="mt-1 text-sm text-muted-foreground">
						{authState.connected
							? `Connected as ${authState.user?.name} (${authState.user?.email}) • ${songs.length} audio files available`
							: 'Connect your Google Drive to stream and manage your personal music collection directly in Echo Music.'}
					</p>
				</div>
			</div>
			...
			<div class="flex flex-wrap items-center gap-3">
				{#if !authState.connected}
					...
					>
						<HugeiconsIcon icon={CloudIcon} class="h-5 w-5" />
	...
	{#if !authState.connected}
		<div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/80 bg-card/40 p-12 text-center backdrop-blur-md">
			...
			<h2 class="text-lg font-semibold text-foreground">Google Drive Not Connected</h2>
	...
	{:else if songs.length === 0}
		<div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/80 bg-card/40 p-12 text-center backdrop-blur-md">
			...
			<h2 class="text-lg font-semibold text-foreground">No Audio Files Discovered</h2>
			<p class="mt-1 max-w-md text-xs text-muted-foreground">
